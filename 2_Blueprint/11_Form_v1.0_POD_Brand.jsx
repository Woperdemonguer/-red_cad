...
const blocks = [
  ...
  },
  {
    ...
    questions: [
      { id: "4.1", q: "Autoevaluación de madurez por ámbitos.", type: "info", description: "En el formulario final se incluirá una matriz tipo semáforo (🔴 necesita apoyo / 🟡 en desarrollo / 🟢 consolidado) para: Planificación productiva · Gestión comercial · Costes de producción · Logística · Calidad · Digitalización · Gobernanza interna · Marketing · Administración · Restauración colectiva." },
      ...
      <div style={{ position: "fixed", bottom: 0, left: 0, right: 0, background: COLORS.white, borderTop: `1px solid ${COLORS.border}`, padding: "12px 20px", zIndex: 10 }}>
        <div style={{ maxWidth: 640, margin: "0 auto", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          ...
          {currentBlock < blocks.length - 1 ? (
            <button onClick={() => setCurrentBlock(currentBlock + 1)} style={{ padding: "10px 24px", borderRadius: 8, border: "none", background: COLORS.forest, color: COLORS.white, fontSize: 14, cursor: "pointer", fontFamily: "system-ui, sans-serif", transition: "all 0.2s" }} onMouseEnter={e => e.target.style.background = COLORS.forestLight} onMouseLeave={e => e.target.style.background = COLORS.forest}>
              Siguiente →
            </button>
          ) : (
            <button onClick={() => setSubmitted(true)} style={{ padding: "10px 24px", borderRadius: 8, border: "none", background: COLORS.accent, color: COLORS.white, fontSize: 14, cursor: "pointer", fontFamily: "system-ui, sans-serif" }}>
              Enviar ✓
            </button>
          )}
        </div>
